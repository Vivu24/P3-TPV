...
Game::Game() {
	...
	//ufoSpawnTime = GetRandomRange(10, 20);
...
// Disparar
void Game::FireLaser(Point2D<int> position, const char* color) { 
...
// Colisiones
bool Game::Damage(SDL_Rect rect, const char* c) {
...
// Carga de mapas
void Game::LoadMaps(string map) {
	// Flujo de entrada
	ifstream entrada(map);

	if (!entrada.is_open()) {
		throw "load map \""s + map + '"';
	}
...
void Game::HasDied(const std::list<SceneObject*>::iterator& it) {
